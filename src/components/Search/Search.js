...
function Search(props) {
  ...
  return (
    <div
      className={`Search ${props.class} ${searchIsOpen && props.openedClass}`}
    >
      <Sprite
        src={search}
        click={openSearch}
      ...
      {searchIsOpen ? (
        <div className="Search__container">
          ...
          {props.label ? (
            <label
              onClick={props.labelFunc}
              className="Search__label"
              htmlFor={props.class ? props.class : "Search__input"}
            >
              {props.label}
            </label>
          ) : (
